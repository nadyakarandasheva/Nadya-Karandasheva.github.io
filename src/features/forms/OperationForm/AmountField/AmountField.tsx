...
export type AmountFieldProps = Pick<CreateOperationFormProps, 'disabled' | 'autoFocusElement'> & {
  submitCount: number;
  touched: boolean;
  errors: string;
  value: number;
...
export const AmountField = memo<AmountFieldProps>(
  ({ onChange, onBlur, onPressEnter, autoFocusElement, touched, value, errors, disabled, submitCount }) => {
    ...
    return (
      <FormItem title="Сумма" required validateStatus={validateStatus} help={help}>
        <Input
          disabled={disabled}
